...
onAuthStateChanged(auth, (user) => {
  ...
  // Re-run protection logic when auth state changes
...
function hideGoogleLoginOverlay() {
  const gOverlay = document.getElementById("google-login-overlay");
...
const initPasswordProtection = async () => {
  ...
  // If we reach here, user is logged in but page is encrypted.
  // Check for saved password or show password entry.
  ...
  async function revealContent(contentHtml: string, password: string) {
  ...
  async function unlock(password: string) {
    try {
      ...
      await revealContent(contentHtml, password);
    } catch {
      const errorMsg = document.getElementById("password-error")
  ...
  // Show password overlay if not already revealed
